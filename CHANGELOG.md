...
- ルーン選択画面にテスト用の「全装備」を追加した。4種類のルーンを同時に付与し、各ルーンの既存Console発動ログで動作を確認できる。
- A長押し中の通常攻撃射程表示と左クリック地点から最寄りの敵（タワー除外）への攻撃指示、地点・対象指定スキルの射程外自動接近、成立した移動スキルの共通通知を追加した。
- 攻撃属性をDamageTypeとは独立した5種類の複数フラグとして扱う将来方針を設計書に記録した。属性とシールド等のスキル属性は未実装。
...
- リネスQ/RとリーゼロッテQ/E/Rで、キーを押している間に射程または方向を表示する。
- 共通Dの成功時反撃をダメージ0・強制スタン0.75秒（初期値）に変更し、リネスR外周ではDを消費しないようにした。FのLayerMaskはキャラクターQの設定へ依存しない。QWERの手動スキルはキー解放時に発動し、Sは発動前の自動接近を解除する。
- 追撃ルーンは実際に成立した自己移動スキルまたはFを起点とし、敵ヒーローへの命中だけで発動する。スロウは命中時の現在移動速度を0.85倍にする。

### Fixed

- 追撃ルーンの旧Eダメージ除外処理と「発動しません」ログを削除した。移動スキル後の最初の敵ヒーロー命中で1回発動し、装備時のログは表示しない。
- HPゲージの透明な左右余白を除き、朧Rの処刑閾値マーカーが緑ゲージの正確な10%位置を示すようにした。
- リーゼロッテQは選択済み対象も発動対象として使い、対象への接近中に右クリックの長押しで中断されないようにした。
...
- リネスWの対象を範囲内の敵プレイヤーに限定し、範囲・命中・スネアの仮表示を追加した。Eの持続中表示を追加した。
- 敵プレイヤーのHPバー上へ、付与元に依存しないスタン・スネア表示を追加した。位置と見やすさは手動確認待ち。
- `SC_Prototype`の`TrainingDummy`を`Character`分類にした場合、対プレイヤー検証用の代役としてリネスWとスタン・スネア表示の対象にした。
- HPバー上のスタン・スネア表示を拡大し、文字の縦方向クリップを解除して暗い背景を追加した。表示位置・見やすさは再度手動確認待ち。

### Fixed

- CC付与時にも対象HPバーの状態管理を結び直し、生成順による通知の取り逃しを補う。ダミー自身の`HardCcTestEmitter`は、Target未指定時にダミー自身を優先する。実画面での表示は手動再確認待ち。

## 2026-09-24

### Added

- リネス（Rines）の初期プロトタイプを追加。専用CharacterData・Prefab Variant・仮マテリアル、P/Q/W/E/R、選択画面とHUD・ルーン・死亡時処理への接続を実装した。数値と専用演出は未確定で、手動確認後にTASKS.mdの親項目を完了にする。

## 2026-08-12

### Changed

- TASKS.mdの「朧の全段階統合テストを行う」を完了へ更新した。
- 朧Qへ`20 + AD×50%`の通常ダメージを追加した。Zelf Qの`30 + AD×60%`、Volbraak Qの`25 + AD×80%`を基準に、貫通・2ストック・テレポートを持つことを考慮して低めに設定した。
- 朧Qの既定Cast ModeをQuickCastへ変更し、Q押下時に即時発射するようにした。Groundを取得できない場合も水平面または正面方向へフォールバックする。
- 朧Qの投射高さをPlayerルートと同じ0、接触半径を0.60へ変更し、敵の近接・遠隔ミニオンへ命中するようにした。
- 朧Wの効果時間を3秒へ変更した。攻撃・スキル・再発動などの既存解除条件が先に成立した場合はその時点で終了する。
- 朧Rの処刑閾値を最大HP10%以下へ変更し、発動ダメージを対象最大HP10%分の確定ダメージへ変更した。

### Fixed

- 朧Wの効果時間を、旧Prefab/Inspectorのシリアライズ値に依存しない固定定数3秒として再実装した。発動時刻を1回取得し、効果終了時刻と残り時間を必ず3秒から開始する。
- PlayerSpawnerで朧選択時のスポーンワールドY座標を1へ固定し、OboroSkillInstallerにも生成直後のY=1補正を追加した。スポナー経由・既存シーンへの直接配置の両方で高いY座標を引き継がない。
- 朧E/Rの対象を敵チャンピオン(Character分類)へ統一した。TeamMemberを持たないTrainingDummyでも、TargetableのClassificationをCharacterへ変更すればテスト用敵チャンピオンとして発動対象になるよう修正した。
...
- ヒーローのチームが全員ブルーに割り当てられ、キルポイント・シャットダウン報酬が発生しない問題を修正した(レーンのどちら側にいるかでブルー/レッドを割り当て、キル不成立時は調査用ログを出すようにした)。
- テストプレイ用: トレーニングダミー・攻撃ダミー(RespawnControllerを持つ非ヒーロー)を倒した場合もキルポイント(25pt)を付与するようにした(ダミーは倒されるたびに連続キル数が1増える扱いで、2回目以降の撃破でシャットダウン報酬+10/+20/+30ptを順に確認できる)。

### Changed
- 本拠地を廃止し、各チームのタワーを2本にした(2本目は旧本拠地位置X=±33・HP5,000で1本目が破壊されるまで無敵)。2本目のタワーが破壊されたチームの負けとなる。
- タワーのHPバーに段階報酬(1,000ダメージ毎)の境界線を表示するようにした。
...
- Lv1〜Lv6のポイント閾値を実装(LevelSystem)。Lv2:40 / Lv3:90 / Lv4:150 / Lv5:225 / Lv6:310で、レベルアップに必要なポイントは徐々に増える。ポイントHUDにチームレベルを併記。
